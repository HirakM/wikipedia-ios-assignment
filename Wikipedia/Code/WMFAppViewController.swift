...
final class WMFAppViewController: UITabBarController, AppTabBarDelegate {
    ...
    private func configureTabController() {
        ...
        } else if selectedIndex != WMFAppTabType.places.rawValue {
    ...
    func processUserActivity(_ activity: NSUserActivity, animated: Bool, completion done: @escaping () -> Void) -> Bool {
        ...
        case .places:
            ...
            } else if let latitude = activity.wmf_placesLatitude()?.doubleValue,
                      let longitude = activity.wmf_placesLongitude()?.doubleValue {
                placesViewController.updateViewModeToMap()
                placesViewController.showLocation(
                    at: CLLocationCoordinate2D(latitude: latitude, longitude: longitude),
                    name: activity.wmf_placesName()
                )
            }
