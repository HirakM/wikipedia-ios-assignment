...
+ (instancetype)wmf_placesActivityWithURL:(NSURL *)activityURL {
    ...
    for (NSURLQueryItem *item in components.queryItems) {
        if ([item.name isEqualToString:@"WMFArticleURL"] && item.value.length > 0) {
            articleURL = [NSURL URLWithString:item.value];
        } else if ([item.name isEqualToString:@"lat"] || [item.name isEqualToString:@"latitude"]) {
            latitude = @([item.value doubleValue]);
        } else if ([item.name isEqualToString:@"long"] || [item.name isEqualToString:@"longitude"] || [item.name isEqualToString:@"lon"]) {
            longitude = @([item.value doubleValue]);
        } else if ([item.name isEqualToString:@"name"] && item.value.length > 0) {
            name = item.value;
        }
    }
...
- (nullable NSNumber *)wmf_placesLatitude {
    return self.userInfo[@"WMFPlacesLatitude"];
}

- (nullable NSNumber *)wmf_placesLongitude {
    return self.userInfo[@"WMFPlacesLongitude"];
}

- (nullable NSString *)wmf_placesName {
    return self.userInfo[@"WMFPlacesName"];
}
