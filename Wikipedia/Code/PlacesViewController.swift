...
class PlacesViewController: ArticleLocationCollectionViewController, UISearchBarDelegate, ArticlePopoverViewControllerDelegate, PlaceSearchSuggestionControllerDelegate, NSFetchedResultsControllerDelegate, UIPopoverPresentationControllerDelegate, ArticlePlaceViewDelegate, UIGestureRecognizerDelegate {
    ...
    /// When set, avoid recentering the map on the device location.
    fileprivate var shouldIgnoreUserLocationUpdates = false
    ...
    /// Centers the map on the given coordinate (used by `wikipedia://places?lat=&long=`).
    @objc public func showLocation(at coordinate: CLLocationCoordinate2D, name: String?) {
        guard view != nil else { // force view instantiation
            return
        }
        shouldIgnoreUserLocationUpdates = true
        panMapToNextLocationUpdate = false
        let location = CLLocation(latitude: coordinate.latitude, longitude: coordinate.longitude)
        zoomAndPanMapView(toLocation: location)
        if let name, !name.isEmpty {
            navigationItem.searchController?.searchBar.placeholder = name
        }
    }
...
extension PlacesViewController {
    override func locationManager(_ locationManager: LocationManagerProtocol, didUpdate location: CLLocation) {
        ...
        guard panMapToNextLocationUpdate, !shouldIgnoreUserLocationUpdates else {
            return
        }
        panMapToNextLocationUpdate = false
        zoomAndPanMapView(toLocation: location)
    ...
    override func locationManager(_ locationManager: LocationManagerProtocol, didUpdateAuthorized authorized: Bool) {
        ...
        if authorized {
            panMapToNextLocationUpdate = currentSearch == nil && !shouldIgnoreUserLocationUpdates
            locationManager.startMonitoringLocation()
        } else {
            panMapToNextLocationUpdate = false
            locationManager.stopMonitoringLocation()
